...
from rltools.rltools import nn, tfutil, util
...
def make_ngstep_func(model, compute_obj_kl, compute_obj_kl_with_grad, compute_hvp_helper):
    ...
    def wrapper(sess, feed, max_kl, damping, subsample_hvp_frac=.1, grad_stop_tol=1e-6,
        ...
        try:
            assert np.allclose(kl0, 0.,
                               atol=1e-6), 'Initial KL divergence is %.7f, but should be 0' % (kl0)
        except AssertionError as e:
            util.warn('Initial KL divergence is %.7f, but should be 0' % (kl0))
